...
根据 QSDT **公理 I 和 II**，宇宙是一个由空间量子 $\mathcal{Q}$ 构成的实体网络。即使在没有任何物质激发态（即没有粒子）的情况下，这个网络本身也并非“一无所有”。它具有内在的结构、连接和量子涨落。
...
因此，**QSDT 理论自然地预言了宇宙常数的存在，其物理本质就是空间量子网络自身的基态能量密度。**
...
1.  **为何其密度不随宇宙膨胀而稀释？**
    ...
    * **QSDT 的解释**: 空间膨胀在 QSDT 理论中有着清晰的微观图像——**新的空间量子 $\mathcal{Q}$ 单元在不断地创生**。由于暗能量是空间本身的属性（每个 $\mathcal{Q}$ 单元都贡献一份基态能量），那么每当空间“生长”出一份新的体积，也就必然“长出”一份新的基态能量。因此，**能量的总量和空间的总体积以完全相同的比例增加，导致其能量密度 $\rho_{\text{vac}}$ 始终保持为一个常数**。
...
这个缺失的部分是物理上非常自然的：空间量子 $\mathcal{Q}$ 自身可能就带有一份**固有的、正的“裸”能量 $E_0$**。这可以被看作是创建一单位空间本身所需要的能量成本。
...
**重要洞见**：我们发现，如果QSDT宇宙的真空能量要为零，那么其基本参数之间必须满足一个**精确的关系**。即，一个空间量子固有的裸能量 $E_0$，必须与其相互作用强度 $J$ 精确地匹配。
